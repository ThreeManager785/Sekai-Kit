...
private struct FrontendTests {
    ...
    func testCharacter() async throws {
        let allBirthdays = try #require(await _DoriAPI.Character.allBirthday())
        let sortedBirthdays = allBirthdays.sorted(by: { $0.birthday < $1.birthday })
        for (index, birthday) in sortedBirthdays.enumerated() {
            let birthdaysInExactBirthday = try #require(await _DoriFrontend.Character.recentBirthdayCharacters(aroundDate: birthday.birthday))
            ...
            let recentBirthdays = try #require(await _DoriFrontend.Character.recentBirthdayCharacters(aroundDate: dateAfter))
