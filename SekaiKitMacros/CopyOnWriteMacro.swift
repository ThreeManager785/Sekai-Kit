...
public struct _CopyOnWriteVarPeerImplMacro: PeerMacro {
    public static func expansion(
        ...
        var result = VariableDeclSyntax(
            ...
            .var,
            name: "__13SekaiKitMacros28_CopyOnWriteVarPeerImplMacroV_\(raw: binding.pattern.cast(IdentifierPatternSyntax.self).identifier.text)"
        )
...
public struct _CopyOnWriteVarAccessorImplMacro: AccessorMacro {
    public static func expansion(
        ...
        let decl = declaration.cast(VariableDeclSyntax.self)
        let binding = decl.bindings.first!
        let wrapperName: PatternSyntax = "__13SekaiKitMacros28_CopyOnWriteVarPeerImplMacroV_\(raw: binding.pattern.cast(IdentifierPatternSyntax.self).identifier.text)"
        ...
        if node.operator.is(AssignmentExprSyntax.self),
           (node.leftOperand.is(DeclReferenceExprSyntax.self)
            ...
            if let leftRef = node.leftOperand.as(DeclReferenceExprSyntax.self) {
                let newLeftRef = DeclReferenceExprSyntax(baseName: "__13SekaiKitMacros28_CopyOnWriteVarPeerImplMacroV_\(leftRef.baseName)")
                return ExprSyntax(
                    node
                ...
                let newLeftRef = MemberAccessExprSyntax(
                    ...
                    period: .periodToken(),
                    declName: DeclReferenceExprSyntax(baseName: "__13SekaiKitMacros28_CopyOnWriteVarPeerImplMacroV_\(leftDecl.baseName)")
                )
                return ExprSyntax(
                    node
                        .with(\.leftOperand, ExprSyntax(newLeftRef))
