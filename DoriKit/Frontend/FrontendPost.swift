...
extension DoriAPI.Posts.PagedPosts: AsyncSequence {
    ...
    public struct AsyncIterator: AsyncIteratorProtocol {
        ...
        internal var request: DoriAPI.Posts.ListRequest
        internal var buffer: [Element]
        
        internal init(count: Int, request: DoriAPI.Posts.ListRequest, currentElements: [Element]) {
        ...
        public mutating func next() async throws -> DoriAPI.Posts.Post? {
            defer {
                position += 1
                request.offset += 1
            }
            if position >= buffer.count {
                if request.offset <= request.limit {
                    if let newList = await DoriAPI.Posts._list(request) {
