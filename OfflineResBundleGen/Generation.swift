...
func generateLocale(_ locale: _DoriAPI.Locale, to output: URL, startTime: TimeInterval = CFAbsoluteTimeGetCurrent()) async throws {
    let info = await retryUntilNonNil { await _DoriAPI.Assets.info(in: locale) }
    var finishedCount = 0
...
private func generateFromInfo(
    _ info: _DoriAPI.Assets.AssetList,
    ...
    for (name, child) in info {
        ...
        case .files:
            ...
            LimitedTaskQueue.shared.addTask {
                var contents: [String]!
                for i in 0..<5 {
                    if let result = await _DoriAPI.Assets._contentsOf(_path + name, in: locale) {
                        contents = result
                        break
                    } else if i == 4 {
...
private func fileCount(of info: _DoriAPI.Assets.AssetList) -> Int {
    var result = 0
    for (_, child) in info {
        switch child {
