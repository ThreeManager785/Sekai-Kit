...
extension RichContentGroup {
    public static func resolveMarkdownImage(
        ...
        if label == "%%SekaiKit_Resolve_Emoji%%" {
...
private func compileRichContent(_ content: RichContent) -> String {
    ...
    case .emoji(let emoji):
        #if canImport(AppKit) && !targetEnvironment(macCatalyst)
        " ![%%SekaiKit_Resolve_Emoji%%](greatdori://\(emoji.image.tiffRepresentation?.base64EncodedString() ?? "")) "
        #else
        " ![%%SekaiKit_Resolve_Emoji%%](greatdori://\(emoji.image.pngData()?.base64EncodedString() ?? "")) "
        #endif
    }
}
