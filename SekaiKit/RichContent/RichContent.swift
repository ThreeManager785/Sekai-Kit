...
/// > This API is currently in development and is unstable.
/// > It is subject to change, and software implemented with this API should be tested with its stable version.
public enum RichContent: Sendable, Equatable, Hashable, SekaiCache.Cacheable {
    case br
    case text(String)
    case heading(String)
    case bullet(String)
...
extension RichContentGroup {
    ...
    @usableFromInline
    internal init(_ newsContent: [SekaiAPI.News.Item.Content]) {
        let dateFormatter = DateFormatter()
        dateFormatter.dateStyle = .medium
        dateFormatter.timeStyle = .short
        func parseSection(_ section: [SekaiAPI.News.Item.Content.ContentDataSection], ul: Bool = false) -> Self {
            var result = Self()
            for data in section {
                switch data {
