...
extension SekaiFrontend.User {
    public typealias LoginCredential = SekaiAPI.User.LoginCredential
    public typealias Token = SekaiAPI.User.Token
    ...
    public final class CredentialManager: @unchecked Sendable {
        ...
        private static let tokenTag = "com.memz233.SekaiKit.Frontend.CredentialManager.Token"
        private static let usernameTag = "com.memz233.SekaiKit.Frontend.CredentialManager.Username"
        private static let passwordTag = "com.memz233.SekaiKit.Frontend.CredentialManager.Password"
        ...
        private init() {
            ...
            if unsafe SecItemCopyMatching(tokenReadQuery as CFDictionary, &_token) == errSecSuccess,
               ...
               let tokenString = String(data: data, encoding: .utf8) {
                let expirationDate = UserDefaults.standard.double(forKey: "_SekaiKit_UserTokenExpirationDate")
                if expirationDate > 0 {
        ...
        public func updateToken(_ token: borrowing Token) {
            ...
            UserDefaults.standard.set(
                ...
                forKey: "_SekaiKit_UserTokenExpirationDate"
            )
        }
        
        public func renewToken(_ token: consuming Token) async -> Token {
            if let username, let password {
                (try? await SekaiAPI.User.login(
...
public func withUserToken<R>(
    _ token: SekaiFrontend.User.Token?,
    _ body: () throws -> R
) rethrows -> R {
    try SekaiFrontend.User.$_currentUserToken.withValue(token, operation: body)
}
...
public func withUserToken<R>(
    ...
    try await SekaiFrontend.User.$_currentUserToken.withValue(token, operation: body)
}
