...
// MARK: extension SekaiFrontend
extension SekaiFrontend {
    /// A type that can be sorted by ``SekaiFrontend/Sorter``.
    public protocol Sortable {
        /// A group of ``SekaiFrontend/Sorter/Keyword`` that can be used
        /// for sorting this type.
        static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] { get }
        ...
        ///     of ``SekaiFrontend/Sorter/Keyword/localizedString(hasEndingDate:)``
        ///     to get more accurate description of a keyword if needed.
        static var hasEndingDate: Bool { get }
        
        static func _compare<ValueType>(usingSekaiSorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool?
    }
    ...
    public struct Sorter: Sendable, Equatable, Hashable, Codable {
        ...
        public enum Keyword: RawRepresentable, CaseIterable, Sendable, Equatable, Hashable, Codable {
            case releaseDate(in: SekaiAPI.Locale)
            case difficultyReleaseDate(in: SekaiAPI.Locale)
            case mvReleaseDate(in: SekaiAPI.Locale)
            case level(for: SekaiAPI.Songs.DifficultyType)
            case rarity
            case maximumStat
            case id
            ...
            public init?(rawValue: UInt32) {
                ...
                switch high {
                case 1 << 0:
                    if let locale = SekaiAPI.Locale(rawIntValue: Int(low)) {
                ...
                case 1 << 3:
                    if let difficulty = SekaiAPI.Songs.DifficultyType(rawValue: Int(low)) {
...
extension Set<SekaiFrontend.Sorter.Keyword> {
    @inlinable
    public func sorted() -> [SekaiFrontend.Sorter.Keyword] {
        self.sorted { $0.rawValue < $1.rawValue }
    }
}

// MARK: extension PreviewEvent
extension SekaiAPI.Events.PreviewEvent: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiAPI.Events.PreviewEvent, let castedRHS = rhs as? SekaiAPI.Events.PreviewEvent else { return nil }
...
extension SekaiAPI.Gachas.PreviewGacha: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
        [.releaseDate(in: .jp), .releaseDate(in: .en), .releaseDate(in: .tw), .releaseDate(in: .cn), .releaseDate(in: .kr), .id]
    }
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiAPI.Gachas.PreviewGacha, let castedRHS = rhs as? SekaiAPI.Gachas.PreviewGacha else { return nil }
        switch sorter.keyword {
        case .releaseDate(let locale):
            return sorter.strictCompare(
...
extension SekaiFrontend.Cards.CardWithBand: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiFrontend.Cards.CardWithBand, let castedRHS = rhs as? SekaiFrontend.Cards.CardWithBand else { return nil }
        switch sorter.keyword {
        case .releaseDate(let locale):
            return sorter.strictCompare(
...
extension SekaiFrontend.Cards.PreviewCard: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
        [.releaseDate(in: .jp), .releaseDate(in: .en), .releaseDate(in: .tw), .releaseDate(in: .cn), .releaseDate(in: .kr), .rarity, .maximumStat, .id]
    }
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiFrontend.Cards.PreviewCard, let castedRHS = rhs as? SekaiFrontend.Cards.PreviewCard else { return nil }
        switch sorter.keyword {
        case .releaseDate(let locale):
            return sorter.strictCompare(
...
extension SekaiAPI.Songs.PreviewSong: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiAPI.Songs.PreviewSong, let castedRHS = rhs as? SekaiAPI.Songs.PreviewSong else { return nil }
        switch sorter.keyword {
        case .releaseDate(let locale):
            return sorter.strictCompare(
                castedLHS.publishedAt.forLocale(locale)?.corrected(),
...
extension SekaiAPI.LoginCampaigns.PreviewCampaign: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
        [.releaseDate(in: .jp), .releaseDate(in: .en), .releaseDate(in: .tw), .releaseDate(in: .cn), .releaseDate(in: .kr), .id]
    }
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiAPI.LoginCampaigns.PreviewCampaign, let castedRHS = rhs as? SekaiAPI.LoginCampaigns.PreviewCampaign else { return nil }
        switch sorter.keyword {
        case .releaseDate(let locale):
            return sorter.strictCompare(
                castedLHS.publishedAt.forLocale(locale)?.corrected(),
...
extension SekaiAPI.Comics.Comic: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
        [.id]
    }
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiAPI.Comics.Comic, let castedRHS = rhs as? SekaiAPI.Comics.Comic else { return nil }
        switch sorter.keyword {
//        case .releaseDate(let locale):
//            return sorter.compare(
...
extension SekaiFrontend.Costumes.PreviewCostume: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
        [.releaseDate(in: .jp), .releaseDate(in: .en), .releaseDate(in: .tw), .releaseDate(in: .cn), .releaseDate(in: .kr), .id]
    }
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiFrontend.Costumes.PreviewCostume, let castedRHS = rhs as? SekaiFrontend.Costumes.PreviewCostume else { return nil }
        switch sorter.keyword {
        case .releaseDate(let locale):
            return sorter.strictCompare(
                castedLHS.publishedAt.forLocale(locale)?.corrected(),
...
extension SekaiFrontend.Characters.PreviewCharacter: SekaiFrontend.Sortable {
    @inlinable
    public static var applicableSortingTypes: [SekaiFrontend.Sorter.Keyword] {
        [.id]
    }
    ...
    public static func _compare<ValueType>(usingSekaiSorter sorter: SekaiFrontend.Sorter, lhs: ValueType, rhs: ValueType) -> Bool? {
        guard let castedLHS = lhs as? SekaiFrontend.Characters.PreviewCharacter, let castedRHS = rhs as? SekaiFrontend.Characters.PreviewCharacter else { return nil }
        switch sorter.keyword {
        case .id:
            return sorter.compare(castedLHS.id, castedRHS.id)
        default:
...
extension Array where Element: SekaiFrontend.Sortable {
    public func sorted(withSekaiSorter sorter: SekaiFrontend.Sorter) -> [Element] {
    ...
    public mutating func sort(withSekaiSorter sorter: SekaiFrontend.Sorter) {
