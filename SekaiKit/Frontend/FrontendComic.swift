...
extension SekaiFrontend {
    ...
    /// use ``SekaiAPI/Comics/Comic/type`` to get its type.
    ///
    ...
    public enum Comics {
        ...
        public static func list() async -> [Comic]? {
            guard let comics = await SekaiAPI.Comics.all() else { return nil }
            return comics
        }
    }
}

extension SekaiFrontend.Comics {
    public typealias Comic = SekaiAPI.Comics.Comic
}

extension SekaiAPI.Comics.Comic {
    @inlinable
    public init?(id: Int) async {
        guard let all = await SekaiAPI.Comics.all() else { return nil }
...
extension SekaiAPI.Comics.Comic {
    /// Type of a ``SekaiAPI/Comic/Comic``.
    @frozen
