...
extension SekaiFrontend {
    ...
    public enum LoginCampaigns {
        ...
        public static func list() async -> [PreviewCampaign]? {
            guard let campaigns = await SekaiAPI.LoginCampaigns.all() else { return nil }
            return campaigns
        }
    }
}

extension SekaiFrontend.LoginCampaigns {
    public typealias PreviewCampaign = SekaiAPI.LoginCampaigns.PreviewCampaign
}
