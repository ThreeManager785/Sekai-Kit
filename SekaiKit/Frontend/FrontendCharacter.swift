...
extension SekaiFrontend {
    ...
    /// See ``SekaiAPI/Characters/Character/nickname`` for more details.
    ///
    ...
    public enum Characters {
        ...
        public static func recentBirthdayCharacters(aroundDate date: Date = .now, timeZone: TimeZone = .init(identifier: "Asia/Tokyo")!) async -> [BirthdayCharacter]? {
            ...
            let allBirthday = await SekaiAPI.Characters.allBirthday() ?? (SekaiCache.PreCache.isAvailable ? SekaiCache.preCache.birthdayCharacters : nil)
        ...
        public static func categorizedCharacters() async -> CategorizedCharacters? {
            ...
            var result = [SekaiAPI.Bands.Band?: [PreviewCharacter]]()
        ...
        public static func extendedInformation(of id: Int) async -> ExtendedCharacter? {
            let groupResult = await withTasksResult {
                await SekaiAPI.Characters.detail(of: id)
            } _: {
...
extension SekaiFrontend.Characters {
    ...
    public typealias CategorizedCharacters = [SekaiAPI.Bands.Band?: [PreviewCharacter]]
    public typealias Character = SekaiAPI.Characters.Character
    
    /// Represent an extended character.
    public struct ExtendedCharacter: Sendable, Identifiable, Hashable, SekaiCache.Cacheable {
        ...
        public var costumes: [SekaiAPI.Costumes.PreviewCostume]
        /// Events that contain this character.
        public var events: [SekaiAPI.Events.PreviewEvent]
        ...
        internal init(
            id: Int,
            character: Character,
            band: SekaiAPI.Bands.Band?,
            cards: [SekaiAPI.Cards.PreviewCard],
            costumes: [SekaiAPI.Costumes.PreviewCostume],
            events: [SekaiAPI.Events.PreviewEvent],
            gacha: [SekaiAPI.Gachas.PreviewGacha]
        ) {
            self.id = id
...
extension SekaiFrontend.Characters.ExtendedCharacter {
    @inlinable
    public init?(id: Int) async {
        if let character = await SekaiFrontend.Characters.extendedInformation(of: id) {
            self = character
        } else {
            return nil
        }
...
extension SekaiFrontend.Characters.ExtendedCharacter {
    ...
    /// - Returns: A random card of this character.
    @inlinable
    public func randomCard() -> SekaiAPI.Cards.PreviewCard? {
