...
nonisolated(unsafe) private var assetList: SekaiAPI.Assets.AssetList?
        ...
        if fullParent.is(DeclReferenceExprSyntax.self) {
            ...
            if memberAccess.declName.baseName == token || token.text == "." {
                if let decl = memberAccess.base?.as(DeclReferenceExprSyntax.self) {
                    ...
                    if let accessParent = SekaiKit.fullParent(of: memberAccess) {
                  ...
                  let stringParent = SekaiKit.fullParent(of: stringLiteral),
...
internal func _prepareAssetListForZeileCompletion() -> Bool {
    ...
    Task.detached {
        if let list = await SekaiAPI.Assets.info(in: .jp) {
...
private func lookupPath(
    ...
    if !path.hasPrefix("/") {
        ...
        var pathDesc = SekaiAPI.Assets.PathDescriptor(locale: .jp)
        func resolveChild(
            _ pathSeg: [String],
            in list: SekaiAPI.Assets.AssetList
        ) -> SekaiAPI.Assets.Child? {
        ...
        case .files:
            ...
            if let c = bundleFileListCacheLock
                ...
                let desc = pathDesc
                Task { @Sendable in
                    contents = await SekaiAPI.Assets.contentsOf(desc)
                    semaphore.signal()
                }
                semaphore.wait()
                if let contents {
