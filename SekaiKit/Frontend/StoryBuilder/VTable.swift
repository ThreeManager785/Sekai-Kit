...
private func _characterName(byID id: Int, in ctx: IRGenEvaluator) -> String {
    if let character = SekaiCache.preCache.characters.first(
        where: { $0.id == id }
    ) {
        ...
        return extendMap[id]?.forLocale(ctx.sema.locale) ?? ""
    }
}
