...
extension SekaiFrontend {
    ...
    public enum Misc {
        ...
        public static func extendedItems<T>(
            ...
            guard let texts = await SekaiAPI.Misc.itemTexts() else { return nil }
            ...
            var degrees: [SekaiAPI.Degrees.Degree]?
            var decoFrames: [SekaiAPI.Misc.DecoFrame]?
            var decoPins: [SekaiAPI.Misc.DecoPin]?
            var decoPinSets: [SekaiAPI.Misc.DecoPinSet]?
            var gameLaneSkins: [SekaiAPI.Misc.GameLaneSkin]?
            var stamps: [SekaiAPI.Misc.Stamp]?
            var cards: [SekaiAPI.Cards.PreviewCard]?
            
            var result = [ExtendedItem]()
            for item in items {
                var text: SekaiAPI.Misc.ItemText?
                ...
                case .star:
                    ...
                    iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/common_rip/star.png")
                case .coin:
                    text = .init(
                        name: .init(
                            jp: "コイン",
                    ...
                    iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/common_rip/coin.png")
                case .stamp, .voiceStamp:
                    ...
                    if stamps == nil && failureFlag & 1 << 5 == 0 {
                        let list = await SekaiAPI.Misc.stamps()
                        if let list {
                            stamps = list
                        } else {
                            failureFlag |= 1 << 5
                    ...
                    if let stamps,
                       let stamp = stamps.first(where: { $0.id == item.itemID }) {
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/stamp/01_rip/\(stamp.imageName).png")
                        
                        if item.type == .voiceStamp {
                            relatedItemSource = .stampVoice(URL(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/sound/voice_stamp_rip/\(stamp.imageName).mp3")!)
                        }
                    }
                case .degree:
                    if degrees == nil && failureFlag & 1 == 0 {
                        let list = await SekaiAPI.Degrees.all()
                        if list != nil {
                            degrees = list
                        } else {
                            failureFlag |= 1
                    ...
                    iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/common_rip/degree.png")
                case .michelleSeal:
                    text = .init(
                        name: .init(
                            jp: "ミッシェルシール",
                    ...
                    iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/common_rip/michelle_seal.png")
                case .decoFrame:
                    if decoFrames == nil && failureFlag & 1 << 1 == 0 {
                        let list = await SekaiAPI.Misc.decoFrames()
                        if list != nil {
                            decoFrames = list
                        } else {
                            failureFlag |= 1 << 1
                    ...
                    if let decoFrames,
                       let frame = decoFrames.first(where: { $0.id == item.itemID }) {
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/deco/frame_rip/\(frame.assetBundleName).png")
                    } else {
                        text = .init(
                            name: .init(
                                jp: "フレーム",
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/deco/frame_rip/deco_frame007.png")
                    }
                case .decoPins:
                    if decoPins == nil && failureFlag & 1 << 2 == 0 {
                        let list = await SekaiAPI.Misc.decoPins()
                        if list != nil {
                            decoPins = list
                        } else {
                            failureFlag |= 1 << 2
                    ...
                    if let decoPins,
                       let pin = decoPins.first(where: { $0.id == item.itemID }) {
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/deco/pins_rip/\(pin.assetBundleName).png")
                    } else {
                        text = .init(
                            name: .init(
                                jp: "ピンズ",
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/deco/pins_rip/deco_pins_single.png")
                    }
                case .decoPinsSet:
                    if decoPinSets == nil && failureFlag & 1 << 3 == 0 {
                        let list = await SekaiAPI.Misc.decoPinSets()
                        if list != nil {
                            decoPinSets = list
                        } else {
                            failureFlag |= 1 << 3
                    ...
                    if let decoPinSets,
                       let pinSet = decoPinSets.first(where: { $0.id == item.itemID }) {
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/deco/pins_rip/\(pinSet.assetBundleName).png")
                    } else {
                        text = .init(
                            name: .init(
                                jp: "ピンズセット",
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/deco/pins_rip/deco_pins_shuffle.png")
                    }
                case .inGameSkinLane:
                    if gameLaneSkins == nil && failureFlag & 1 << 4 == 0 {
                        let list = await SekaiAPI.Misc.gameLaneSkins()
                        if list != nil {
                            gameLaneSkins = list
                        } else {
                            failureFlag |= 1 << 4
                    ...
                    if let gameLaneSkins,
                       let skin = gameLaneSkins.first(where: { $0.id == item.itemID }) {
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/liveskinlane_rip/\(skin.assetBundleName).png")
                    } else {
                        text = .init(
                            name: .init(
                                jp: "レーンスキン",
                        ...
                        iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/liveskinlane_rip/skin05.png")
                    }
                case .situation:
                    if cards == nil && failureFlag & 1 << 6 == 0 {
                        let list = await SekaiAPI.Cards.all()
                        if list != nil {
                            cards = list
                        } else {
                            failureFlag |= 1 << 6
                ...
                case .costume3DMakingItem:
                    ...
                    iconImageURL = .init(string: "https://bestdori.com/assets/\(SekaiAPI.preferredLocale.rawValue)/thumb/material_rip/costume3dmakingitem001.png")
                default: break
                }
                result.append(.init(
                    item: item,
        ...
        public static func extendedPlayerProfile(of id: Int, in locale: SekaiAPI.Locale) async -> ExtendedPlayerProfile? {
            let groupResult = await withTasksResult {
                await SekaiAPI.Misc.playerProfile(of: id, in: locale)
            } _: {
...
extension SekaiFrontend {
    public typealias Item = SekaiAPI.Item
    
    public struct ExtendedItem: Sendable, Identifiable, Hashable, SekaiCache.Cacheable {
        public var item: Item
        public var text: SekaiAPI.Misc.ItemText?
        ...
        public enum ItemSource: Sendable, Hashable, SekaiCache.Cacheable {
            case card(SekaiAPI.Cards.PreviewCard)
            case stampVoice(URL)
        }
    }
}

extension SekaiFrontend.Misc {
    public struct ExtendedPlayerProfile: Sendable, Hashable, SekaiCache.Cacheable {
        public var profile: SekaiAPI.Misc.PlayerProfile
        public var degrees: [SekaiAPI.Degrees.Degree]
        public var keyVisualCard: SekaiAPI.Cards.PreviewCard
        public var mainDeckCards: [SekaiAPI.Cards.PreviewCard]
        public var songs: [SekaiAPI.Songs.PreviewSong]
    }
}

extension SekaiAPI.Misc.StoryAsset {
